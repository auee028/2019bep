...
int main(void)
{
	...
	Mat img_valley(height, width, CV_8UC1);
	...
	float v_max = -1;
	for (int pix = 0; pix < grayscale; pix++) {
		...
		sigma_b_sqrt = (1 - arr_pdf[pix]) * powf((uT * w0) - temp, 2) / (w0 * w1);
	...
	for (int j = 0; j < height; j++) {
		for (int i = 0; i < width; i++) {
			if (img_gray.at<uchar>(j, i) > threshold) img_valley.at<uchar>(j, i) = 255;
			else img_valley.at<uchar>(j, i) = 0;
		}
	}
	...
	imshow("valley emphasis", img_valley);
	imshow("histogram", img_histo);
	//imwrite("valley_original.jpg", img_gray);
	//imwrite("valley_emphasis.jpg", img_valley);		// Don't miss .jpg
	//imwrite("valley_histogram.jpg", img_histo);
